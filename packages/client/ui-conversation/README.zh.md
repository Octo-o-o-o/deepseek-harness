...
草稿最开头、且没有选区时按 ArrowUp，按从新到旧召回本会话自己发过的提问；ArrowDown 向新的方向走回，越过最新一条后恢复开始召回时正在写的草稿。触发菜单打开时箭头归菜单所有；只有菜单把按键交回来，它才表示召回。召回一旦开始，光标位置不再参与判断，因此连续 ArrowUp 会持续后退，而不会卡在被召回的多行提问内部；停在最旧一条不再回绕。编辑草稿或发送都会结束这次召回。steering 消息同样计入提问，因为那也是用户敲的；纯图片与纯空白的提问会跳过，前后相邻的重复提问会合并。召回读取已加载的对话节点，因此只能回溯到已加载窗口的边界；只读或处于 pending lock 的输入框会拒绝召回。
