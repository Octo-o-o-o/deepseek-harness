...
describe('Enter semantics', () => {
  ...
  it('keeps the owning placeholder or ordinary guidance when whole-queue steering is unavailable', () => {
    expect(bench({ running: true }).textarea.placeholder).toBe('给智能体发消息，Shift+Enter 换行')
    expect(bench({ queue: [row('q-1')] }).textarea.placeholder).toBe('给智能体发消息，Shift+Enter 换行')
    expect(bench({ running: true, queue: [row('q-1')], draft: '消息' }).textarea.placeholder).toBe('给智能体发消息，Shift+Enter 换行')
    expect(bench({
      running: true,
      queue: [row('q-1')],
      subagent: {
    ...
    }).textarea.placeholder).toBe('给智能体发消息，Shift+Enter 换行')
    expect(bench({
      running: true,
      queue: [row('q-1')],
      placeholder: '上层指定提示',
...
describe('running and lock semantics', () => {
  ...
  it('disabled state shows the unavailable placeholder; custom placeholder wins', () => {
    ...
    expect(textarea.placeholder).toBe('会话不可用')
    const live = bench()
    expect(live.textarea.placeholder).toBe('给智能体发消息，Shift+Enter 换行')
  ...
  it('the plan projection swaps the placeholder while its effective target is plan mode', () => {
    ...
    expect(leaving.textarea.placeholder).toBe('给智能体发消息，Shift+Enter 换行')
...
describe('prompt recall', () => {
  ...
  it('sending ends the walk', () => {
    ...
    expect(sink).toHaveBeenCalledWith('second ask', [], 'queue', expect.any(AbortSignal))
    caretAt(textarea, 0)
    act(() => { fireEvent.keyDown(textarea, { key: 'ArrowUp' }) })
    expect(textarea.value).toBe('second ask')
  })
