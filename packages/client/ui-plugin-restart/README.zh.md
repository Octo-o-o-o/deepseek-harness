...
仅桌面可见的插件：当 profile 的插件清单已经不同于运行中组合所读到的内容时，出现一个重启入口；同时为通用品牌 slot（`sidebar.brand.mark`、`sidebar.brand.name`、`conversation.hero.brand.mark`）填入打包应用的 occupant。`dsh plugin add` 会改写 profile manifest 的 `dsh.profile.bundles`，但组合在启动时只取一次该清单——`composeLive` 只重读 patch 文件——因此新装的插件在应用重新启动之前始终不生效。node 半侧是一个空 `apply`，只为让插件出现在 Loader 中；浏览器半侧经 `exports["./client"]` 交付。
...
该插件由壳自己的 patch 层激活（`apps/desktop/desktop.patch.yml`，以 `--patch` 传入），绝不经由共用的 web 组合包，因此 `npx @deepseek-ai/dsh web` 的组合与此前完全一致。品牌 occupant 因此不会替换浏览器标签页上的官方或 Local Build 回退。

## Model Experience

无，因为该插件只贡献浏览器控件、没有 host 侧行为；这里没有任何东西会到达模型请求。
