...
The plugin is activated by the shell's own patch layer (`apps/desktop/desktop.patch.yml`, passed as `--patch`), never by the shared web bundle, so `npx @deepseek-ai/dsh web` composes exactly as before. The brand occupants therefore never replace the official or local-build fallbacks of a browser tab.

## Model Experience

None, as the plugin contributes browser controls and no host behavior; nothing here reaches a model request.
