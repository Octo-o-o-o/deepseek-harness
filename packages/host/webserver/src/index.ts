 ...
 * single admission-guard seat every request passes before routing, the
 * structured index injection table with raw transform taps behind it, and the
 * single fallback seat for everything no route claims). Knows no harness
 * concepts and serves no files; the composing
