...
export function apply(ctx: Context, config: Config): void {
  ...
  // The desktop shell already hosts the GUI in a WebView; opening the OS
  // browser would hand a nonce-gated loopback URL to an unpaired tab.
  const handoffBrowser = config.openBrowser && !launchedThroughSsh(ctx) && desktop === undefined
  ...
  if (desktop !== undefined || config.printUrl || handoffBrowser) {
