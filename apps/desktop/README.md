...
- **A plugin that registers an `/api` route prevents startup.** Desktop mode refuses any `/api` registration it does not own, because `match()` gives an exact route precedence over the connection prefix and the desktop token is checked inside connection's own handlers — so a third-party exact route would serve unauthenticated data on a loopback port that carries no user identity. The refusal is fatal to the whole Loader, so one plugin takes the app down and the shell only reports `sidecar stdout closed before the ready line`; the real cause is in `$DSH_HOME/logs/sidecar.log`. Remove the plugin's `insert` from the profile patch to recover.
