...
impl SidecarProcess {
    ...
    pub fn shutdown(&mut self, grace: Duration) {
        ...
        join_with_deadline(self.readers.drain(..).collect(), READER_JOIN_TIMEOUT);
    }
}
...
/// How long shutdown waits for a log reader after cancellation before
/// leaving it detached: quitting the application must not depend on a
/// grandchild closing a pipe.
